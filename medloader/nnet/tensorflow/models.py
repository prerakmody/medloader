...
class ModelUNet3DShallow(tf.keras.Model):

    def __init__(self, class_count, activation='softmax', trainable=False, verbose=False):
        super(ModelUNet3DShallow, self).__init__(name='ModelUNet3DShallow')
        ...
        if 1:
            ...
            self.convblock3 = ConvBlock3D(filters=filters[2]  , pool=False, dropout=dropout[2], trainable=trainable, name='Block3') # Dim/4 (e.g. 96/4=24)

            self.upconvblock4 = UpConvBlock3D(filters=filters[1][0], trainable=trainable, name='Block4_1')
            self.convblock4   = ConvBlock3D(filters=filters[1], pool=False, dropout=dropout[3], trainable=trainable, name='Block4_2') 
            
            self.upconvblock5 = UpConvBlock3D(filters=filters[0][0], trainable=trainable, name='Block5_1')
            self.convblock5   = ConvBlock3D(filters=filters[0], pool=False, dropout=dropout[4], trainable=trainable, name='Block5_2')
